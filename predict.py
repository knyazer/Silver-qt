...
def seq2crops(seq, step=1):
  ...
  for path, img, im0s, vid_cap in seq:
    ...
    pred = torch.tensor(sess.run([sess.get_outputs()[0].name], {sess.get_inputs()[0].name: img}))
    pred = [x.numpy() for x in non_max_suppression(pred, 0.25, 0.45, None, False, max_det=1000)]
    if len(pred[0]) != 0:
      ...
      for det in pred:
        w, h = det[3] - det[1], det[2] - det[0]
        sz = max(w, h)
        xc, yc = (det[3] + det[1]) / 2.0, (det[2] + det[0]) / 2.0
        x0, y0, x1, y1 = xc - sz / 2, yc - sz / 2, xc + sz / 2, yc + sz / 2

        x0 = ir(max(0, min(x0, im0s.shape[0])))
        x1 = ir(max(0, min(x1, im0s.shape[0])))

        y0 = ir(max(0, min(y0, im0s.shape[1])))
        y1 = ir(max(0, min(y1, im0s.shape[1])))

        crops.append(cv.resize(im0s[x0:x1, y0:y1, :], (32, 32)))
      res.append(crops)
  return res
